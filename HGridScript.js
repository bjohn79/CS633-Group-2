...
function ChangePlusMinusText(PlusMinusCellID)
{
	try
	{
	    ...
	    //var PlusMinusCellObj = document.getElementById(PlusMinusCellID);
	    PlusMinusCellObj = document.getElementById(PlusMinusCellID);
		if ( PlusMinusCellObj != null )
		{
		    if (PlusMinusCellObj.innerHTML.indexOf("grid-arrow-down.gif") > -1)
			{
			    PlusMinusCellObj.innerHTML = "<img src='../images/phobosweb/grid-arrow-up.gif'>";
				
			}
			else
			{
			    PlusMinusCellObj.innerHTML = "<img src='../images/phobosweb/grid-arrow-down.gif'>";
			}
		}
	}
	catch(e)
...
function ShowExpandedDivInfo(TextBoxIDOfExpandedDiv, DataGridID)
{
	try
	{
	    var DIVTB = document.getElementById(TextBoxIDOfExpandedDiv);
		if ( DIVTB != null )
		{	
			...
			if ( data != null && data.length > 0 && data != ",")
			{
				...
				for ( var i = 0; i < SplitData.length; i++)
				{
					...
					{
						//Expand DIV Section
						ShowPanel( DivIdentifier,count );
					}
					
					if ( PlusMinusCellIdentifier != null && PlusMinusCellIdentifier.length > 0 )
					{
						var PlusMinusCellObj = document.getElementById(PlusMinusCellIdentifier);
						if ( PlusMinusCellObj != null )
						{
						...
						else
						{
							...
							{
								var NewCellObject = document.getElementById(NewPlusMinusCellObjID);
								if ( NewCellObject != null )
								{
									...
									if ( rows != null && rows.length >= 0)
									{
										...
										try
										{
										    rows[0].innerHTML = "<img src='../images/phobosweb/grid-arrow-up.gif'>";
										}
										catch(e)
										{
...
function SetExpandedDIVInfo(PlusMinusCellID, HiddenTextBoxToHoldDivInfo, DIVInfoID)
{
	try
	{
		...
		var HTBObj = document.getElementById(HiddenTextBoxToHoldDivInfo);
		var HLObj = document.getElementById(PlusMinusCellID);
		
		if ( HLObj != null && HTBObj != null)
		{

		    if (HLObj.innerHTML == "<img src='../images/phobosweb/grid-arrow-down.gif'>")
			{
				IsExpanded = false;	
			}
			else
...
function HidePanel(Panel,count)
{
	try
	{
		var ChosenPanel = document.getElementById(Panel);
		if ( ChosenPanel != null )
		{
			document.getElementById(Panel).style.display = "none";
		}
		showHideImage(count, false);
	}
	catch(e)
	{
...
function ShowPanel(Panel, count)
{
	try
	{
		var ChosenPanel = document.getElementById(Panel);
...
function showHideImage(count,showHide) {
	var i = 0;
	var cellID;
	for (i = 0; i <= count+1; i++) {
		var j = i + 2;
		if (j > 9) {
			cellID = 'ctl00_maincontent_gvstudents_ctl' + j + '_CellInfo' + i;

		}
		else {
			cellID = 'ctl00_maincontent_gvstudents_ctl0' + j + '_CellInfo' + i;
		}
		if (showHide) {
			document.getElementById(cellID).innerHTML = "<img src='../images/phobosweb/grid-arrow-up.gif'>";
		}
		else {
			document.getElementById(cellID).innerHTML = "<img src='../images/phobosweb/grid-arrow-down.gif'>";
		}
		//document.getElementById('ctl00_maincontent_gvstudents_ctl03_CellInfo2').innerHTML = "<img src='../images/phobosweb/grid-arrow-up.gif'>";
	}
}
...
function HideShowPanel(Panel)
{
	try
	{
		...
		if ( ChosenPanel != null )
		{
			var currentdisplay = document.getElementById(Panel).style.display;
...
//added by lava kumar
function ExpandCollapse(DataGridID, count) {
	if (document.getElementById('ctl00_maincontent_hdnexpand').value == '1') {

		ShowAll(DataGridID, count);
        document.getElementById('ctl00_maincontent_hdnexpand').value = "0";
    }
    else {
    
    	HideAll(DataGridID, count);
        document.getElementById('ctl00_maincontent_hdnexpand').value = "1";

    }
    return false;
}
function ShowAll(DataGridID, count) {

    var j = 0;
    for (j = 0; j < count; j++) {
        var dataid = "uniquename" + j;
        ShowPanel(dataid, count);
    }
}

function HideAll(DataGridID,count) {

    var j = 0;
    for (j = 0; j < count; j++) {
        var dataid = "uniquename" + j;
        HidePanel(dataid,count);
    }
}
